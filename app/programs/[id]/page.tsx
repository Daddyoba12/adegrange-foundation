...
export default function ProgramDetailPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">Program Not Found</h1>
  ...
  return (
    <div className="min-h-screen py-8 md:py-16 px-4">
      <div className="container mx-auto max-w-6xl">
        ...
        <Link 
          href="/programs" 
          className="inline-flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white mb-6 md:mb-8 transition-colors text-sm md:text-base"
        >
  ...
  {program.description}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mt-6 md:mt-8">
            <div className="bg-white dark:bg-gray-900 rounded-lg p-3 md:p-4 border border-gray-200 dark:border-gray-800">
              <div className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-1">Year</div>
              <div className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">
                {program.year}
              </div>
            </div>
            <div className="bg-white dark:bg-gray-900 rounded-lg p-3 md:p-4 border border-gray-200 dark:border-gray-800">
              <div className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-1">Location</div>
              <div className="text-lg md:text-xl lg:text-2xl font-bold text-gray-900 dark:text-white break-words leading-tight">
                {program.location}
              </div>
            </div>
            <div className="bg-white dark:bg-gray-900 rounded-lg p-3 md:p-4 border border-gray-200 dark:border-gray-800 col-span-2">
              <div className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-1">Beneficiaries</div>
              <div className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">
        ...
        <section className="mb-8 md:mb-12">
          <h2 className="text-2xl md:text-3xl font-bold mb-3 md:mb-4">About the Program</h2>
  ...
  {program.fullDescription}
        ...
        {/* Impact Section */}
        <section className="mb-8 md:mb-12">
          <h2 className="text-2xl md:text-3xl font-bold mb-4 md:mb-6">Key Impact</h2>
          <div className="grid gap-3 md:gap-4">
            {program.impact.map((item, index) => (
              <div 
                key={index}
                className="flex items-start gap-3 bg-white dark:bg-gray-900 rounded-lg p-3 md:p-4 border border-gray-200 dark:border-gray-800"
              >
                <div className="flex-shrink-0 w-5 h-5 md:w-6 md:h-6 bg-green-500 rounded-full flex items-center justify-center mt-0.5">
                  <svg className="w-3 h-3 md:w-4 md:h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <p className="text-sm md:text-base text-gray-700 dark:text-gray-200">{item}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-8 md:mb-12">
          <div className="flex justify-between items-center mb-4 md:mb-6">
            <h2 className="text-2xl md:text-3xl font-bold">Gallery Preview</h2>
          </div>
          
          {loading ? (
            <div className="text-center py-8 md:py-12 text-sm md:text-base">Loading...</div>
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm md:text-base">
              No images available for this program yet.
            </p>
          )}
        </section>
        ...
        {media.documents.length > 0 && (
          <section className="mb-8 md:mb-12">
            <h2 className="text-2xl md:text-3xl font-bold mb-4 md:mb-6">Reports & Documents</h2>
            <div className="grid gap-3 md:gap-4">
              {media.documents.map((doc) => (
                <a
                  ...
                  className="flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 transition-colors"
                >
                  <div className="text-3xl md:text-4xl">📄</div>
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-sm md:text-base truncate">{doc.file_name}</p>
                    <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Click to view</p>
                  </div>
                </a>
              ))}
            </div>
        ...
        {selectedImage && (
          ...
          >
            <button
              className="absolute top-2 right-2 md:top-4 md:right-4 text-white text-5xl md:text-4xl hover:text-gray-300 w-12 h-12 md:w-auto md:h-auto flex items-center justify-center"
              onClick={() => setSelectedImage(null)}
              aria-label="Close"
            >
              ×
