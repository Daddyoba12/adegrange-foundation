...
import { Upload, FileText, BookOpen, LogOut, CheckCircle, AlertCircle, Image as ImageIcon } from 'lucide-react'
...
function MediaUpload() {
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const { error: upErr } = await supabase.storage
          .from(bucket).upload(storagePath, file, { upsert: true, cacheControl: '3600' })
        if (upErr) throw upErr

        const { error: dbErr } = await supabase.from('program_media').insert({
          program_id: selectedProgram, media_type: mediaType,
          media_url: storagePath, file_name: file.name,
          folder_name: folderName, display_order: i
        })
    ...
    setUploading(false)
    setMessage(`✅ Done! ${ok} uploaded${fail > 0 ? `, ${fail} failed` : ''}.`)
    setFiles(null);
    ...
    <form onSubmit={handleUpload} className="space-y-5">
      ...
      <Field label="Select Files *">
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5">
          Accepted: jpg, png, gif, webp, pdf, doc, docx
        </p>
      </Field>
      ...
      <SubmitBtn loading={uploading} label="Upload Files" loadingLabel={`Uploading... ${progress}%`} />
    </form>
  )
}
...
function AddBlogPost() {
  const empty = {
    title: '', slug: '', category: '', author: '', authorRole: '',
    excerpt: '', content: '', readTime: '', imageUrl: '', tags: '',
    featured: false, date: new Date().toISOString().split('T')[0]
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (imgFile) {
      ...
      const { error: upErr } = await supabase.storage
        .from('program-images').upload(path, imgFile, { upsert: true })
      if (upErr) {
    ...
    const { error } = await supabase.from('blog_posts').insert({
      ...
      category: form.category, author: form.author, author_role: form.authorRole,
      date: form.date, read_time: parseInt(form.readTime) || 5, image_url: imageUrl,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <label className="flex items-center gap-3 cursor-pointer select-none p-3 rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
        <input type="checkbox" checked={form.featured} onChange={e => set('featured', e.target.checked)}
          className="w-4 h-4 rounded border-gray-300 text-pink-600 focus:ring-pink-500" />
        <span className="text-sm text-gray-700 dark:text-gray-300 font-medium">Mark as Featured post</span>
      </label>

      {message && <StatusBanner msg={message} />}
      <SubmitBtn loading={saving} label="Publish Blog Post" loadingLabel="Publishing..." />
    </form>
  )
}
...
function AddProgram() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('programs').insert({
      ...
      year: parseInt(form.year), location: form.location,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Year *">
          <input type="number" min="2000" max="2099" value={form.year}
            onChange={e => set('year', e.target.value)} placeholder="2024" className={inputCls} required />
        </Field>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Beneficiaries Reached">
          <input type="number" min="0" value={form.beneficiaries}
            onChange={e => set('beneficiaries', e.target.value)} placeholder="150" className={inputCls} />
        </Field>
      ...
      <Field label="Description">
        <textarea rows={4} value={form.description} onChange={e => set('description', e.target.value)}
          placeholder="Describe the programme goals, activities, and outcomes..."
          className={`${inputCls} resize-y`} />
      </Field>

      <div className="flex items-start gap-3 p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
        <ImageIcon size={16} className="text-blue-600 dark:text-blue-400 mt-0.5 flex-shrink-0" />
        <p className="text-xs text-blue-700 dark:text-blue-300">
          After adding a program, go to the <strong>Media Upload</strong> tab to upload photos and documents for it.
        </p>
      </div>

      {message && <StatusBanner msg={message} />}
      <SubmitBtn loading={saving} label="Add Program" loadingLabel="Saving..." />
    </form>
  )
}
...
const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
  { id: 'media',   label: 'Media Upload',  icon: <Upload size={18} /> },
  { id: 'blog',    label: 'Blog Post',     icon: <BookOpen size={18} /> },
  { id: 'program', label: 'Add Program',   icon: <FileText size={18} /> },
]
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 bg-gray-50 dark:bg-gray-950">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold
                       ...
                       hover:bg-red-50 dark:hover:bg-red-900/20 active:scale-95 transition"
          >
            <LogOut size={14} />
            <span>Logout</span>
          </button>
        ...
        {/* Tab bar — icons always visible, labels shown on sm+ */}
        <div className="flex rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-800 mb-6 bg-white dark:bg-gray-900">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2
                          py-3 sm:py-3.5 text-xs sm:text-sm font-semibold transition-colors ${
                activeTab === tab.id
        ...
        {/* Panel card */}
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-4 sm:p-6 lg:p-8">
